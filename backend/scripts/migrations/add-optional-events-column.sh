...
echo "Adding optional_events column to people table in database: $db_name"

action_sql="SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = '$db_name' AND TABLE_NAME = 'people' AND COLUMN_NAME = 'optional_events';"
if [[ -n "$db_pass" ]]; then
...
if [[ "$COLUMN_EXISTS" -gt 0 ]]; then
  echo "✓ optional_events column already exists in people table"
  exit 0
fi

if [[ -n "$db_pass" ]]; then
  MYSQL_PWD="$db_pass" mysql "${MYSQL_ARGS[@]}" "$db_name" <<SQL
ALTER TABLE people ADD COLUMN IF NOT EXISTS optional_events tinyint(1) not null default 0 AFTER scratch_masters;
SQL
else
...
if [[ -n "$db_pass" ]]; then
  MYSQL_PWD="$db_pass" mysql "${MYSQL_ARGS[@]}" "$db_name" <<SQL
ALTER TABLE people
  ADD COLUMN IF NOT EXISTS optional_best_3_of_9 tinyint(1) not null default 0 AFTER optional_events,
  ADD COLUMN IF NOT EXISTS optional_scratch tinyint(1) not null default 0 AFTER optional_best_3_of_9,
  ADD COLUMN IF NOT EXISTS optional_all_events_hdcp tinyint(1) not null default 0 AFTER optional_scratch;
SQL
else
  mysql "${MYSQL_ARGS[@]}" "$db_name" <<SQL
ALTER TABLE people
...
echo "✓ Successfully added optional_events column to people table"
