...
# Add nickname column to people table
# This migration adds the nickname field introduced in the nickname feature
...
if [[ -z "${PORTAL_DATABASE_URL:-}" ]]; then
  ...
  echo "Please set PORTAL_DATABASE_URL in your .env.local file"
  exit 1
fi
...
echo "Adding nickname column to people table in database: $db_name"
...
if [[ "$COLUMN_EXISTS" -gt 0 ]]; then
  echo "✓ nickname column already exists in people table"
  echo "No migration needed."
  exit 0
fi

echo "Adding nickname column..."

if [[ -n "$db_pass" ]]; then
  MYSQL_PWD="$db_pass" mysql "${MYSQL_ARGS[@]}" "$db_name" <<SQL
ALTER TABLE people ADD COLUMN nickname text AFTER last_name;
SQL
else
...
echo "✓ Successfully added nickname column to people table"
echo ""
